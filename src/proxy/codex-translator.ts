  ...
  for (const raw of messages) {
    ...
    if (role === "tool") {
      const callId =
        ...
        "";
      input.push({ type: "function_call_output", call_id: callId, output: normalizeTextContent(msg.content) });
      continue;
    }
    ...
    for (const tcRaw of toolCalls) {
      ...
      input.push({ type: "function_call", call_id: callId, name: fn.name, arguments: args });
    }
  }
  return input;
}
...
export function openaiToCodexResponses(body: Record<string, unknown>, stream: boolean): Record<string, unknown> {
  ...
  if (mappedTools) { out.tools = mappedTools; out.tool_choice = "auto"; out.parallel_tool_calls = true; }
  if (typeof body.temperature === "number") out.temperature = body.temperature;
  // chatgpt.com/backend-api/codex/responses rejects max_output_tokens / max_tokens — omit them.
  return out;
}
...
function extractResponseTextAndTools(response: Record<string, unknown>): { text: string; toolCalls: unknown[] } {
  ...
  for (const itemRaw of output) {
    ...
    if (type === "function_call") {
      ...
      toolCalls.push({ id: callId, index: toolIndex++, type: "function", function: { name, arguments: args } });
    }
  }
...
export function translateCodexNonStream(raw: Record<string, unknown>): Record<string, unknown> {
  ...
  const out: Record<string, unknown> = {
    ...
    model: typeof response.model === "string" ? response.model : "gpt-4o",
...
export function newCodexStreamState(): CodexStreamState {
  return {
    id: "", model: "", roleSent: false, completed: false,
    sawTextDelta: false, sawToolDelta: false, nextToolIndex: 0,
...
function roleChunk(state: CodexStreamState): string {
  return `data: ${JSON.stringify({ ...chunkBase(state), choices: [{ index: 0, delta: { role: "assistant" }, finish_reason: null }] })}\n\n`;
}
...
function toolCallDeltaChunk(state: CodexStreamState, toolDelta: Record<string, unknown>): string {
  return `data: ${JSON.stringify({ ...chunkBase(state), choices: [{ index: 0, delta: { tool_calls: [toolDelta] }, finish_reason: null }] })}\n\n`;
}
...
export function codexChunkToOpenAI(rawLine: string, state: CodexStreamState): string[] {
  ...
  if (type === "response.output_text.delta" || type === "response.refusal.delta") {
    ...
    if (!delta) return [];
    const out: string[] = [];
    if (!state.roleSent) { state.roleSent = true; out.push(roleChunk(state)); }
    state.sawTextDelta = true;
    out.push(`data: ${JSON.stringify({ ...chunkBase(state), choices: [{ index: 0, delta: { content: delta }, finish_reason: null }] })}\n\n`);
    return out;
  }
  ...
  if (type === "response.output_item.added" || type === "response.output_item.done") {
    ...
    const out: string[] = [];
    if (!state.roleSent) { state.roleSent = true; out.push(roleChunk(state)); }
    out.push(toolCallDeltaChunk(state, { index: toolIndex, id: callId, type: "function", function: { name, arguments: args || "" } }));
  ...
  if (type === "response.function_call_arguments.delta" || type === "response.function_call_arguments.done") {
    ...
    const delta = typeof event.delta === "string" ? event.delta : typeof event.arguments === "string" ? event.arguments : "";
    if (!callId || !delta) return [];
    const toolIndex = getToolIndex(state, callId);
    const out: string[] = [];
    if (!state.roleSent) { state.roleSent = true; out.push(roleChunk(state)); }
    out.push(toolCallDeltaChunk(state, { index: toolIndex, function: { arguments: delta } }));
    state.sawToolDelta = true;
  ...
  if (type === "response.completed" || type === "response.done" || type === "response.incomplete") {
    ...
    if (!state.sawTextDelta && text) {
      if (!state.roleSent) { state.roleSent = true; out.push(roleChunk(state)); }
      out.push(`data: ${JSON.stringify({ ...chunkBase(state), choices: [{ index: 0, delta: { content: text }, finish_reason: null }] })}\n\n`);
    }
    const doneChunk: Record<string, unknown> = { ...chunkBase(state), choices: [{ index: 0, delta: {}, finish_reason: finishReason }] };
  ...
  if (type === "error" || type === "response.failed") {
    ...
    state.completed = true;
    return [
      `data: ${JSON.stringify({ ...chunkBase(state), choices: [{ index: 0, delta: {}, finish_reason: "stop" }] })}\n\n`,
      "data: [DONE]\n\n",
    ];
  }
