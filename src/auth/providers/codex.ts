...
export function extractCodexAccountId(accessToken: string, providerData: Record<string, unknown> | null): string | null {
  if (providerData && typeof providerData.accountId === "string" && providerData.accountId) {
    return providerData.accountId;
  }
...
function normalize(tokens: Record<string, unknown>): NormalizedTokens {
  ...
  return {
    ...
    email,
    displayName: email,
    providerData: { idToken: (tokens.id_token as string | undefined) ?? null, accountId },
  };
}
...
function buildAdapter(id: string, originator: string): OAuthAdapter {
  ...
  return {
    ...
    async exchangeCode({ code, redirectUri, codeVerifier }) {
      if (!codeVerifier) throw new Error("OpenAI token exchange failed: missing PKCE code_verifier");
